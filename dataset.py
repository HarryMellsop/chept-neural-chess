...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        expected_mask_size = len(doc) * 0.25
        masked_content_size = random.randint(int(expected_mask_size * 0.75), int(expected_mask_size * 1.25))
        masked_content_start = random.randint(0, len(doc) - masked_content_size)

        prefix = doc[:masked_content_start]
        masked_content = doc[masked_content_start:masked_content_start + masked_content_size]
        suffix = doc[masked_content_start + masked_content_size:]
